...
def create_db_manager():
    return dbManager(mysql_host, mysql_user, mysql_password, mysql_db)
...
def login():
    if request.method == "POST":
        ...
        if db_manager.validate_login(username, password):
            ...
            # Return the access & refresh tokens as part of a JSON response
...
def connect(auth):
    ...
    if rooms[room]["members"] == 0:
        rooms[room]["first_player_sid"] = request.sid
        rooms[room]['first_player_user_id'] = user_id
    # When second player joins set up the data
    elif rooms[room]["members"] == 1:
